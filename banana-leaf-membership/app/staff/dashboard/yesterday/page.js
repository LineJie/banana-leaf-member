...
export default function StaffYesterday() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch("/api/staff/yesterday");
      if (res.status === 401) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col gap-3 mb-6 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="font-display text-2xl text-ink">Input Transaksi Kemaren</h1>
          </div>
          <Link
          ...
          <div className="flex flex-col gap-8">
            <section>
              ...
              <p className="font-body text-xs text-ink/50 mb-4">
                {data.newMembers.length} member baru didaftarkan kemarin.
              </p>
              {data.newMembers.length === 0 && (
                <p className="font-body text-sm text-ink/40">
                  Belum ada member baru yang didaftarkan kemarin.
                </p>
              )}
            ...
            <section>
              ...
              <p className="font-body text-xs text-ink/50 mb-4">
                {data.todayTransactions.length} transaksi tercatat kemarin.
              </p>
              {data.todayTransactions.length === 0 && (
                <p className="font-body text-sm text-ink/40">
                  Belum ada transaksi yang tercatat kemarin.
                </p>
              )}
              <div className="flex flex-col gap-2">
